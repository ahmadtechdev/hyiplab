...
class Converter {
  ...
  static String replaceSpaceToUnderscore(String value) {
    try {
      String formatedString = value.replaceAll(' ', '_');
      String v = formatedString.split(" ").map((str) => str.capitalize).join(" ");
      return v;
  ...
  static String sum(String firstNumber,String secondNumber) {
    double sum = 0.0;
    try{
      double? first =  double.tryParse(firstNumber)??0;
      double? second =  double.tryParse(secondNumber)??0;
     sum = first + second;
    } catch(e){
      sum = 0.0;
    }
    return twoDecimalPlaceFixedWithoutRounding(sum.toString());
  }

  static getPercent(String? progressPercent) {
    double percentValue = double.tryParse(progressPercent??"0.0")??0.0;
    if(percentValue!= 0.0){
...
void printx(Object? object) {
  if (kDebugMode || Environment.DEV_MODE) {
    print(object);
  }
}
